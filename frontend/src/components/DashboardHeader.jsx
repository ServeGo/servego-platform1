      ...
      <div className="flex flex-wrap gap-1 bg-slate-100 border border-slate-200 p-1 rounded-xl">
        {tabs.map(tab => (
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === tab.id ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
          >
            {tab.label}
          </button>
        ))}
